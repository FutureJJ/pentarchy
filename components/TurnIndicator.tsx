...
export default function TurnIndicator() {
  ...
  if (!state) {
    return (
      <span className="mono text-[10px] sm:text-xs uppercase tracking-widest text-ash">
        Cycle 0 · awaiting first tick
      </span>
    );
  }
  ...
  return (
    <span className="mono text-[10px] sm:text-xs uppercase tracking-widest text-ash flex items-center gap-2">
      ...
      <span className="text-ink-soft">
        Cycle {state.cycle} · T-{String(state.turn).padStart(2, "0")} / {maxTurns}
      </span>
      <span className="hidden sm:inline">· {state.season}</span>
      <span className="hidden md:inline">· {pct}% complete</span>
      <span className="hidden lg:inline">· last tick {lastTick}Z</span>
    </span>
  );
}
